...
if (isset($_POST['login'])) {
    ...
    $stmt = $conn->prepare("SELECT user_id, user_name, user_email, user_password, is_locked FROM users WHERE user_email = ? AND user_password = ? LIMIT 1");
    $stmt->bind_param("ss", $email, $password);
    if ($stmt->execute()) {
        $stmt->bind_result($user_id, $user_name, $user_email, $user_password, $is_locked);
        $stmt->store_result();
        if ($stmt->num_rows() == 1) {
            $stmt->fetch();
           
            if ($is_locked == 1) {
                header("location:login.php?error=Your account is locked. Please contact support.");
            } else {
